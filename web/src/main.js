...
function App() {
  ...
  return html`<div className="min-h-screen pb-24 bg-gradient-to-br from-luxe-bg via-luxe-bg to-[#ece6db]">
    ...
    <main className="p-4 max-w-7xl mx-auto space-y-3 fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">${[['Продано', monthSales.summary?.sold_count || 0], ['Выручка', money(monthSales.summary?.revenue)], ['Реальная прибыль', money(monthSales.summary?.profit)], ['Потенциальная прибыль', money(monthSales.summary?.potential_profit)]].map(([t, v]) => html`<div className="premium-card rounded-2xl p-3"><p className="text-xs text-luxe-muted">${t}</p><p className="text-lg font-semibold">${v}</p></div>`)}</div>
