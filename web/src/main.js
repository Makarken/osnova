...
function App() {
  ...
  return html`<div className="min-h-screen pb-24 bg-gradient-to-br from-luxe-bg via-luxe-bg to-[#ece6db]">
    ...
    ${repairCardItem && html`<${RepairItemModal} item=${repairCardItem} close=${() => setRepairCardItem(null)} save=${saveItem} completeRepair=${completeRepair} />`}
...
function ItemModal({ item, close, save, updateStatus, updateShipping, openSale, cancelSale, deleteItem, sendToRepair }) {
  ...
  <div className="grid md:grid-cols-2 gap-2">
    ...
    <label className="text-xs inline-flex items-center gap-2"><input type="checkbox" checked=${boolText(f.listed_vinted) === 'yes'} onChange=${(e) => setF({ ...f, listed_vinted: e.target.checked ? 'yes' : 'no' })}/>Выставлено на Vinted</label>
    <label className="text-xs inline-flex items-center gap-2"><input type="checkbox" checked=${boolText(f.listed_vestiaire) === 'yes'} onChange=${(e) => setF({ ...f, listed_vestiaire: e.target.checked ? 'yes' : 'no' })}/>Выставлено на Vestiaire</label>
